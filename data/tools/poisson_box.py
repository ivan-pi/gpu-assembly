...
circular hole, with the stencil graph that goes with it.
    ...
    python3 poisson_box.py --size 32 32 tg_32
    python3 poisson_box.py --size 64 64 --hole 20 cylinder_64.node
    python3 poisson_box.py --size 32 32 --tile 4 4 tg_128
...
MY Ly (TiledNodeSet in pointclouds.nodeset): the copies join without a
seam, and the stencils are searched over the whole tiling.
...
def main():
    ap = argparse.ArgumentParser(
        ...
        epilog="Lengths are in lattice units. Markers are 0 interior, 6 the wall "
        "of the cylinder.",
    )
    ap.add_argument(
        "output",
